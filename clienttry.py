            ...
            try:
                ...
                if message_data:
                    ...
                    if message.get("type") == "broadcast":
                        if "Order has been taken." in message["message"]:
                            messagebox.showinfo("Order Update", "This order is no longer available.")
                            return  # Stop further prompts for this order

                        # Otherwise, prompt for acceptance
                        response = messagebox.askquestion("Incoming order", "Do you want to take this request?")
                        if response == "yes":
                            try:
                                accept_message = {"type": "accept"}
                                self.client_socket.send(json.dumps(accept_message).encode())
                    ...
                    if message["status"] == "failed":
                        messagebox.showinfo("Order Status", message["message"])  # Show rejection message
                    else:
                        messagebox.showinfo("Order Status", message["message"])  # Show "Your order has been sent to the server!" message

                elif message.get("type") == "details":
                    messagebox.showinfo("Order Accepted", "Order details have been sent to you!")
                    break  # Exit the loop after handling the acknowledgment
